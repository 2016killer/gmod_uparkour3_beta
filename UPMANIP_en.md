...
Parameter WorldLerpHandler: Global callback function, in the format of handler(boneMapping, entOrSnapshot, tarEntOrSnapshot, boneName, newPos, newAng, newScale), which can custom modify interpolation results and must return three values (pos/ang/scale).
...
Parameter scaling: Whether to enable bone scaling synchronization; if true, automatically calls SetBoneScale to set the interpolated scaling ratio.
Parameter LocalLerpHandler: Global callback function, in the format of handler(boneMapping, entOrSnapshot, tarEntOrSnapshot, boneName, newPos, newAng, newScale), which can custom modify interpolation results and must return three values (pos/ang/scale).
